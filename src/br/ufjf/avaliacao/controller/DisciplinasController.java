...
public class DisciplinasController extends GenericController {
	...
	public void exclui(@BindingParam("disciplina") Disciplina disciplina) {
		...
		if (!disciplinaBusiness.disciplinaUsada(disciplina)) {
			...
			Messagebox
					.show("Impossível excluir. A disciplina está associada a alguma turma.");
	}

	@Command
	public void changeEditableStatus(
			@BindingParam("disciplina") Disciplina disciplina) {
		disciplina.setEditingStatus(!disciplina.isEditingStatus());
		refreshRowTemplate(disciplina);
	}

	@Command
	public void confirm(@BindingParam("disciplina") Disciplina disciplina)
			throws HibernateException, Exception {
		DisciplinaBusiness business = new DisciplinaBusiness();
		if (business.cadastroValido(disciplina.getCodDisciplina(),
				disciplina.getNomeDisciplina())
				&& !business.cadastrado(disciplina.getCodDisciplina(),
						disciplina.getNomeDisciplina())) {
	...
	public void cadastra() throws HibernateException, Exception {
		...
		if (!disciplinaBussines.cadastroValido(disciplina.getCodDisciplina(),
				disciplina.getNomeDisciplina())) {
			Messagebox.show("Preencha todos os campos!");
		} else if (disciplinaBussines.cadastrado(disciplina.getCodDisciplina(),
				disciplina.getNomeDisciplina())) {
	...
	@Command("upload")
	// FORMATO DE ENTRADA: codigo disciplina, nome disciplina.
